...
def XmlUnserializer(xmlString):
    ...
    for station in root.findall('pdv'):
        gasStation = gasStationModel.gasStationModel(station.get('id'), station.get('latitude'), station.get(
            'longitude'), station.get('cp'), station.get('pop'), station.get('ville'), station.get('adresse'))

        for price in station.findall('prix'):
            carburant = carburantModel.carburantModel(price.get('id'), price.get(
                'nom'), price.get('valeur'), "stock", price.get('maj'))
            gasStation.carburants.append(carburant)

        for rupture in station.findall('rupture'):
            carburant = carburantModel.carburantModel(rupture.get(
                'id'), rupture.get('nom'), "0", "rupture", rupture.get('debut'))
            gasStation.carburants.append(carburant)

        gasStation.services.append(
            [str(service.text, encoding='utf-8') for service in station.findall('service')])
